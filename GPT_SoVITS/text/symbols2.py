...
# 1. 將所有語言的符號合併到一個大列表中
final_symbols = []
final_symbols.extend([pad])
final_symbols.extend(c)
final_symbols.extend(v)
final_symbols.extend(ja_symbols)
final_symbols.extend(pu_symbols)
final_symbols.extend(list(arpa))
final_symbols.extend(["[", "]"]) # 日文調型
final_symbols.extend(list(ko_symbols))
final_symbols.extend(list(yue_symbols)) 
final_symbols.extend(tw_symbols) # 您定義的台語符號列表

# 2. 【核心修正】使用 set 去除所有重複項，然後排序，確保列表的純淨與順序穩定
symbols = sorted(list(set(final_symbols)))

# 3. (可選) 打印最終長度進行驗證
if __name__ == "__main__":
    print(f"最終合併去重後的符號總數 (Total unique symbols): {len(symbols)}")
