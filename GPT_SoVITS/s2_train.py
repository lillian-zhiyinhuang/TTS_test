...
#             # --- 開始替換的邏輯 ---
#             # 根據是否在 DDP 環境下，取得正確的模型實例
#             model_instance = net_g.module if torch.cuda.is_available() else net_g

#             # 載入預訓練模型的權重字典
#             pretrain_dict = torch.load(hps.train.pretrained_s2G, map_location="cpu")[
#                 "weight"
#             ]
...
#             # 建立一個新的字典，只放入那些在預訓練模型和新模型中都存在，且形狀完全匹配的權重
#             print("正在將預訓練權重載入到新模型中...")
#             tmp_dict = {}
...
#             model_instance.load_state_dict(model_dict, strict=False)
#             print("✅ 已成功跳過不匹配的權重，並載入預訓練模型。")
#             # --- 結束替換的邏輯 ---
#         if (
